...
import { ToastvalueService } from '@/diagnofirm/services/toastvalue.service';
                ...
                <div style="border-radius: 56px; padding: 0.3rem; background: linear-gradient(180deg, var(--primary-color) 10%, rgba(33, 150, 243, 0) 30%)">
                    <div class="w-full bg-surface-0 dark:bg-surface-900 py-20 px-8 sm:px-20" style="border-radius: 53px">
                        <div class="text-center mb-8">
                            ...
                            <div class="text-surface-900 dark:text-surface-0 text-3xl font-medium mb-4">Welcome to Diagnofirm!</div>
                            <span class="text-muted-color font-medium">Sign in to continue</span>
                        </div>

                        <div>
                            <label for="email1" class="block text-surface-900 dark:text-surface-0 text-xl font-medium mb-2">User Name</label>
                            <input pInputText id="username" type="text" placeholder="user name" class="w-full md:w-120 mb-8" [(ngModel)]="username" />
...
export class Login {
    ...
    constructor(private router: Router, private toasterService: ToastvalueService,
        private CDR: ChangeDetectorRef, private notificationService: NotificationService,
        private TokenstorageService: TokenstorageService, private dataService: DataService,
    ...
    onSubmit() {
        ...
        const url = GlobalConstants.Authurl + GlobalConstants.Login;

        this.dataService.addData(url, input).subscribe((response: any) => {
            if (response.status === 'success') {
                this.userdata = response['response']['ref1'];
                localStorage.setItem('username', this.userdata[0].usr_name);
                sessionStorage.setItem('isLoggedIn', 'true');
                this.router.navigate(['/diagnofirm'], { replaceUrl: true }); 
                this.CDR.detectChanges();
            } else {
                this.messageService.add({ severity: 'error', summary: 'Error', detail: 'Invalid credentials' });
                //this.notificationService.showMessage('error', 'Error', 'Invalid credentials');
            }
        });
    }
}
